...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($student_id) || empty($subject_id) || empty($semester_id) || $score === '') {
        ...
        try {
            // Kiểm tra xem đã có điểm cho sinh viên, môn học và học kỳ này chưa (trừ điểm hiện tại)
            ...
            $check_stmt->bindParam(':id', $grade_id);
            $check_stmt->execute();
            ...
            if ($check_stmt->fetchColumn() > 0) {
                ...
                // Cập nhật lại thông tin điểm sau khi sửa
                $stmt = $conn->prepare("SELECT * FROM grades WHERE id = :id");
                $stmt->bindParam(':id', $grade_id);
                $stmt->execute();
                $grade = $stmt->fetch(PDO::FETCH_ASSOC);
        ...
        <div class="col-md-8 offset-md-2">
            <div class="card">
                <div class="card-header bg-primary text-white">
                    <h4 class="mb-0">Chỉnh sửa Điểm</h4>
                </div>
                <div class="card-body">
                    <?php if (!empty($error)): ?>
                    ...
                    <form method="post" action="">
                        <div class="form-group">
                            ...
                            <select class="form-control" id="student_id" name="student_id" required>
                                ...
                                <?php foreach ($students as $student): ?>
                                    <option value="<?php echo $student['student_id']; ?>" <?php echo ($grade['student_id'] == $student['student_id']) ? 'selected' : ''; ?>>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control" id="subject_id" name="subject_id" required>
                                ...
                                <?php foreach ($subjects as $subject): ?>
                                    <option value="<?php echo $subject['subject_id']; ?>" <?php echo ($grade['subject_id'] == $subject['subject_id']) ? 'selected' : ''; ?>>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control" id="semester_id" name="semester_id" required>
                                ...
                                <?php foreach ($semesters as $semester): ?>
                                    <option value="<?php echo $semester['semester_id']; ?>" <?php echo ($grade['semester_id'] == $semester['semester_id']) ? 'selected' : ''; ?>>
                        ...
                        <div class="form-group">
                            ...
                            <input type="number" class="form-control" id="score" name="score" min="0" max="10" step="0.1" value="<?php echo htmlspecialchars($grade['score']); ?>" required>
                        </div>
